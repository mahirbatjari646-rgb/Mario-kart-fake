...
function project(p, cameraX, cameraY, cameraZ, cameraDepth, width, height, roadWidth) {
    ...
    // E RENDESISHME: mbrojtje kunder pjesetimit me zero/negativ (kjo ishte shkaku i rruges "flat")
    let safeZ = transZ > 0.01 ? transZ : 0.01;
    let scale = cameraDepth / safeZ;
    return {
        ...
        transZ: transZ // ruajme distancen reale per clipping ne render()
    };
}
...
function render(theme, currentSegment) {
    ...
    // KODRAT PARALLAX NE SFOND (levizin me ngadale se pista -> ndjesi thellesie shtese)
    let hillColor = theme.type === "volcano" ? "#3d1a0f" : (theme.type === "farm" ? "#c0392b" : "#16a085");
    ctx.fillStyle = hillColor;
    let parallaxShift = (player.x * 300 + player.z * 0.02) % 260;
    ctx.beginPath();
    ctx.moveTo(-260, canvas.height / 2);
    for (let hx = -260 - parallaxShift; hx < canvas.width + 260; hx += 260) {
        ctx.lineTo(hx + 130, canvas.height / 2 - 70 - Math.sin(hx * 0.008) * 25);
        ctx.lineTo(hx + 260, canvas.height / 2);
    }
    ctx.lineTo(canvas.width + 260, canvas.height / 2);
    ctx.lineTo(canvas.width + 260, canvas.height / 2 + 5);
    ctx.lineTo(-260, canvas.height / 2 + 5);
    ctx.fill();
    ...
    for (let n = 0; n < 120; n++) {
        ...
        // KLIPI I SAKTE: kalo segmentin nese eshte prapa/tepe afer kameres, ose eshte "back-face", ose fshihet nga nje segment me i afert
        if (p1.transZ <= cameraDepth || p2.screenY >= p1.screenY || p2.screenY >= maxy) continue;
        ...
        if (idx >= 0 && idx <= 2) {
            ...
            // RRUMBLE STRIPS (rrripa te kuq-bardhe anash rruges - kjo jep ndjesi shpejtesie/3D)
            let rw1 = p1.screenWidth * 1.18;
            let rw2 = p2.screenWidth * 1.18;
            ctx.fillStyle = colorToggle ? "#e74c3c" : "#ecf0f1";
            ctx.beginPath();
            ctx.moveTo(p1.screenX - rw1, p1.screenY);
            ctx.lineTo(p1.screenX + rw1, p1.screenY);
            ctx.lineTo(p2.screenX + rw2, p2.screenY);
            ctx.lineTo(p2.screenX - rw2, p2.screenY);
            ctx.fill();
            ...
            // VIJA E NDERPRERE NE MES (dashed center line - klasike per pistat)
            if (colorToggle) {
                let lw1 = p1.screenWidth * 0.045;
                let lw2 = p2.screenWidth * 0.045;
                ctx.fillStyle = "#f9e79f";
                ctx.beginPath();
                ctx.moveTo(p1.screenX - lw1, p1.screenY);
                ctx.lineTo(p1.screenX + lw1, p1.screenY);
                ctx.lineTo(p2.screenX + lw2, p2.screenY);
                ctx.lineTo(p2.screenX - lw2, p2.screenY);
                ctx.fill();
            }
        }
